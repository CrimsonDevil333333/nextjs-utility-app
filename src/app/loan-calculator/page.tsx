...
const InputSlider = ({ label, unit, value, onChange, min, max, step }: any) => (
  <div>
    ...
    <input
      ...
      value={value}
      onChange={onChange}
      className="mt-1 w-full h-2 bg-gray-200 dark:bg-gray-600 rounded-lg appearance-none cursor-pointer"
    />
  </div>
);
...
const LoanCalculatorPage = () => {
  ...
  const [values, setValues] = useState({
    ...
    interestRate: '8.5', // More realistic default for India
    loanTerm: '20',
  });
  
  // Reset loan amount to a sensible default when currency changes
  useEffect(() => {
  ...
  return (
    <div className="max-w-xl mx-auto p-4 sm:p-6">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
        <h1 className="text-2xl sm:text-3xl font-extrabold text-center mb-2 text-gray-800 dark:text-white">Loan Calculator 🏦</h1>
        
        {/* --- Currency Selector --- */}
        <div className="flex justify-center mb-6">
            <select
                value={currencyCode}
                onChange={(e) => setCurrencyCode(e.target.value as keyof typeof CURRENCIES)}

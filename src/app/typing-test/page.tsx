...
const TypingSpeedTestPage = () => {
  ...
  // --- Word Scrolling Logic ---
  useEffect(() => {
    if (activeWordRef.current && wordsContainerRef.current) {
      const activeWord = activeWordRef.current;
      const container = wordsContainerRef.current;
      // Scroll to keep the active word in the vertical center of the container
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Start the test on the first valid keystroke
    ...
    // Handle spacebar press (word completion)
    if (value.endsWith(' ')) {
      if (userInput.length === 0) return; // Prevent advancing on empty space
    ...
    if (lastChar !== ' ' && value.length > userInput.length) {
      if (currentWord && value.length > currentWord.length && lastChar !== ' ') {
        setErrors(prev => prev + 1); // Penalize for extra characters
      } else if (currentWord && currentWord[value.length - 1] !== lastChar) {
        setErrors(prev => prev + 1); // Penalize for incorrect character
      }
    }
  ...
  const calculateResults = () => {
    ...
    // WPM formula: (all typed characters / 5) / time in minutes
    // We use net WPM which considers only correctly typed words.
  ...
  // --- Render ---
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 mt-4 flex flex-col items-center">
            ...
            <div className="flex items-center gap-x-2">
              {DURATION_OPTIONS.map(duration => (
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-sm font-semibold transition ${testDuration === duration
                ...
                {words.map((word, index) => {
                  ...
                  return (
                    <span
                      ...
                      className={`
                  ${wordStatuses[index] === 'correct' ? 'text-green-500' : ''}
                  ${wordStatuses[index] === 'incorrect' ? 'text-red-500 line-through' : ''}
                  ${wordStatuses[index] === 'untyped' && !isCurrent ? 'text-gray-400' : ''}
                  ${isCurrent ? 'font-bold' : ''}
                            `}
                    >
                      {isCurrent ? (
                        <>
                          {word.split('').map((char, charIndex) => {
                            ...
                            const isOvertyped = userInput.length > word.length && charIndex === word.length - 1;
                            return (
                              <span
                                key={charIndex}
                                className={`
                                      ${isCorrect ? 'text-blue-600 dark:text-blue-400' : ''}
                                      ${isIncorrect ? 'text-red-500 bg-red-500/20 rounded-sm' : ''}
                                      ${!isTyped ? 'text-gray-600 dark:text-gray-400' : ''}
                                          `}
                              >
                                {char}
                              </span>

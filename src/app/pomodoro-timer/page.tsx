...
const PomodoroTimerPage = () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center p-4 transition-colors duration-500 ${
        mode === 'pomodoro' ? 'bg-red-100 dark:bg-red-900/50' : 
        mode === 'shortBreak' ? 'bg-blue-100 dark:bg-blue-900/50' : 
        'bg-green-100 dark:bg-green-900/50'
    }`}>
      <div className="w-full max-w-md mx-auto bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-2xl shadow-2xl p-4 sm:p-8 text-center">
        <h1 className="text-2xl sm:text-3xl font-bold mb-4 text-gray-800 dark:text-gray-100">Pomodoro Timer 🍅</h1>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
            <button onClick={toggleTimer} className={`w-full sm:w-32 px-8 py-4 text-xl sm:text-2xl font-bold text-white rounded-lg shadow-lg transition-transform transform hover:scale-105 ${modeColors[mode]}`}>
